...
namespace fullsail_ai { namespace algorithms {
	...
	private:
		...
		struct Compr {
			bool operator() (PlannerNode* FNode, PlannerNode* SNode) {
				return (FNode->finalCost > SNode->finalCost);
			}
		};
		...
		PriorityQueue<PlannerNode*, Compr> open;

		int HeCost(SearchNode* SNode, SearchNode* FNode) {
			int x = FNode->tile->getColumn() - SNode->tile->getColumn();
			int y = FNode->tile->getRow() - SNode->tile->getRow();
	...
	public:
		...
		// check for a null in nodes - It should not exist any null in nodes
		/*
		bool check() {
